...
function destinationFor(target, name) {
...
const defaultRemovalOperations = {
    rename: renameSync,
    remove: (path) => rmSync(path, { recursive: true, force: true })
};
export function uninstallSkills(target, names, operations = defaultRemovalOperations) {
    ...
    try {
        ...
        for (const { source, backup } of moved.reverse()) {
            if (existsSync(backup))
                operations.rename(backup, source);
        }
        throw error;
    }
    finally {
...
export function installSkills(target, skills) {
