...
export function resolveCachePath(env = process.env, home = homedir()) {
...
export function queryLatestVersion() {
    const output = execFileSync("npm", ["view", PACKAGE_NAME, "version"], {
        encoding: "utf8",
        stdio: ["ignore", "pipe", "ignore"],
        timeout: NPM_TIMEOUT_MS
    }).trim();
    if (!parseSemVer(output))
        throw new Error("npm returned an invalid version.");
    return output;
}
export function checkForUpdate(current, options = {}) {
    const now = options.now?.() ?? Date.now();
    const cachePath = options.cachePath ?? resolveCachePath();
    const readFile = options.readFile ?? ((path) => readFileSync(path, "utf8"));
    const writeFile = options.writeFile ?? ((path, contents) => {
...
export function installLatestVersion(spawn = spawnSync) {
    const result = spawn("npm", ["install", "-g", `${PACKAGE_NAME}@latest`], { stdio: "inherit" });
    return result.error ? 1 : result.status ?? 1;
}
export async function presentUpdate(result, dependencies = {}) {
    if (!result.updateAvailable || !result.latest)
        return 0;
    const log = dependencies.log ?? console.log;
    const approved = await (dependencies.confirm?.(`Update available: ${result.current} -> ${result.latest}. Install now?`) ?? Promise.resolve(false));
    if (!approved) {
